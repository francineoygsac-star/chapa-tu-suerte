...
def init_db():
    ...
    if "document_type" not in columns:
        c.execute("ALTER TABLE orders ADD COLUMN document_type TEXT")
    if "document_number" not in columns:
        c.execute("ALTER TABLE orders ADD COLUMN document_number TEXT")
...
def create_order():
    ...
    try:
        ...
        c.execute(
            "INSERT INTO orders(code,name,phone,document_type,document_number,quantity,total) VALUES(?,?,?,?,?,?,?)",
...
def tickets_by_document():
    ...
    if document_type not in {"DNI","CE"}:
    ...
    c=db()
    rows=c.execute("""
        SELECT o.id,o.code,o.name,o.phone,o.document_type,o.document_number,
               o.quantity,o.total,o.status AS order_status,o.created_at,
               t.number,t.status AS ticket_status
        FROM orders o
        LEFT JOIN tickets t ON t.order_id=o.id
        WHERE o.document_type=? AND o.document_number=?
        ORDER BY o.id DESC, t.number ASC
    """,(document_type,document_number)).fetchall()

    if not rows:
        c.close()
        return jsonify(error="No se encontraron tickets para ese documento"),404
    ...
    for order in orders.values():
        order["status_label"]=status_labels.get(order["status"],"EN REVISIÓN")
        result.append(order)
    ...
    return jsonify(
        ...
        total_tickets=sum(len(x["tickets"]) for x in result),
        orders=result
    )
...
def admin_orders():
    ...
    c=db()
    q="""SELECT id,code,name,phone,document_type,document_number,quantity,total,status,proof_path,created_at,reviewed_at,review_note
         FROM orders"""
    args=[]
    if status!="all":
